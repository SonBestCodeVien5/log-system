...
If the active collaboration mode or permissions forbid file writes, do not claim context was written. Return the exact intended paths and content, mark the context as `not persisted`, and make the next writable phase create those files.
...
Use templates from `.agents/context/templates/`. Do not change application source files during planning unless the user explicitly asks for implementation.
